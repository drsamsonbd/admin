...
<template>
 ...
 <div class="row">
   ...
   <div>
    <b-button size="sm" variant="outline-primary" id="show-btn" @click="showModal">Pembelian Baru</b-button>
 ...
 <!-- Simple Tables -->
              <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Senarai Pembelian</h6>
                </div>
...
<script type="text/javascript">
    ...
    this.allCategory();
     ...
     data(){
      return{
        ...
        fields: [
          { key: 'index', label:'No.'},
          { key: 'finance_code', label: 'Unit', sortable: true, sortDirection: 'asc' },
          { key: 'no', label: 'Pemohon', sortable: true, sortDirection: 'asc' },
          { key: 'descriptions', label: 'Tajuk Permohonan', sortable: true, sortDirection: 'asc' },
          { key: 'nil', label: 'No. Dokumen', sortable: true, sortDirection: 'asc' },
          { key: 'nol', label: 'Status', sortable: true, sortDirection: 'asc' },
  ...
  methods:{
    allCategory(){
    let self = this;
     axios.get('/api/finance_category/'+ '?token='+ localStorage.getItem('token'))
    ...
    ,
       deleteCategory(id){
              ...
              if (result.value) {
                axios.delete('/api/finance_category/'+id+ '?token='+ localStorage.getItem('token'))
               .then(() => {
     ...
     categoryUpdate(){
       ...
       axios.patch('/api/finance_category/'+id+ '?token='+ localStorage.getItem('token'),this.forms)
       .then(() => {
        window.location.reload()
        Notification.success()
       })
